...
public class PersonaControllerV1 {
	...
	public PersonaResponse persona(@PathVariable String database, @PathVariable Integer dni) {
		log.info("Into findOne REST API");
		return personaInputAdapterRest.findPersona(database.toUpperCase(), dni);
	}

	@ResponseBody
	@PutMapping(path = "/{dni}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public PersonaResponse editarPersona(@PathVariable Integer dni, @RequestBody PersonaRequest request) {
		log.info("Into edit REST API");
	...
	public PersonaResponse eliminarPersona(@PathVariable String database, @PathVariable Integer dni) {
