...
public class TelefonoControllerV1 {
	...
	public TelefonoResponse crearTelefono(@RequestBody TelefonoRequest request) {
		log.info("Into create phone REST API");
	...
	public TelefonoResponse telefono(@PathVariable String database, @PathVariable String number) {
		log.info("Into findOne phone REST API");
		return telefonoInputAdapterRest.findTelefono(database.toUpperCase(), number);
	}

	@ResponseBody
	@PutMapping(path = "/{number}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public TelefonoResponse editarTelefono(@PathVariable String number, @RequestBody TelefonoRequest request) {
		log.info("Into edit phone REST API");
		return telefonoInputAdapterRest.editarTelefono(number, request);
	}
	...
	public TelefonoResponse eliminarTelefono(@PathVariable String database, @PathVariable String number) {
		log.info("Into delete phone REST API");
